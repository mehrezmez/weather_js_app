...
navigator.geolocation.getCurrentPosition(function (position) {
    ...
    function main () {
        fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=metric&appid=${keyApi}`).then(function (response) {
            ...
            if (dateTime.getUTCHours() < 10 && dateTime.getUTCMinutes() > 10) {
                currentHour.textContent = `0${dateTime.getUTCHours() + 1}:${dateTime.getUTCMinutes()}`;
            } else if (dateTime.getUTCHours() < 10 && dateTime.getUTCMinutes() < 10) {
                currentHour.textContent = `0${dateTime.getUTCHours() + 1}:0${dateTime.getUTCMinutes()}`;
            } else if (dateTime.getUTCHours() > 10 && dateTime.getUTCMinutes() < 10) {
